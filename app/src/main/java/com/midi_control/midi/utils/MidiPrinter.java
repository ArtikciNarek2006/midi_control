...
public class MidiPrinter {
    ...
    @NonNull
    public static String formatMessage(@NonNull byte[] data, int offset, int count) {
        StringBuilder sb = new StringBuilder();
    ...
    public static String formatDeviceInfo(MidiDeviceInfo info) {
        ...
        if (info != null) {
            ...
            for (MidiDeviceInfo.PortInfo port : info.getPorts()) {
                sb.append((port.getType() == MidiDeviceInfo.PortInfo.TYPE_INPUT) ? "input"
                        : "output");
                sb.append("[").append(port.getPortNumber()).append("] = \"").append(port.getName()).append("\"\n");
            }
        }
        return sb.toString();
    }
